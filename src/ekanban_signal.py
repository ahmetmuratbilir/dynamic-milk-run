...
def main():
    ...
    with open(rapor_yol, "w", encoding="utf-8") as f:
        ...
        f.write(f"| Toplam Sinyal | {len(df)} |\n")
        f.write(f"| Teslim Edilen | {len(df[df['durum']=='TESLİM'])} |\n")
        f.write(f"| Açık Kalan | {len(df[df['durum']=='ACIK'])} |\n")
        f.write(f"| Starvation Olayı | {len(starv)} |\n\n")
